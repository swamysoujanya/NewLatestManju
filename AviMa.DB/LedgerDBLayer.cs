...
namespace AviMa.DataBaseLayer
{
    public class LedgerDBLayer
    {
        ...
        public DataTable GetCredithistory(int CuSuID, char CustOrSupp, string getDate, ref string errorInfo)
        {
            MySqlConnection objCon = new MySqlConnection(ConfigValueLoader.ConnectionString);
            ...
            try
            {
                ...
                if (string.IsNullOrEmpty(getDate)) // get data WRT cust id
                {
                    query = " SELECT SiNo, CuSuID, AccountName, CustOrSupp,DebitAmnt, CreditAmnt,BalanceAmnt, date, User " +
                                   " FROM creditdebithistory where CuSuID = " + CuSuID + " and CustOrSupp = '" + CustOrSupp + "' ";
                }
                else // get data WRT date
                {
                    query = " SELECT SiNo, CuSuID, AccountName, CustOrSupp,DebitAmnt, CreditAmnt,BalanceAmnt, date, User " +
                                 " FROM creditdebithistory where (DATE_FORMAT(date, '%Y-%m-%d')) = '" + getDate + "' and CustOrSupp = '" + CustOrSupp + "' ";
                }
        ...
        public DataTable GetLedgerRprtItemByDate(string rptDate,ref bool openingBalExist, ref int siNoOfCreatedOpeningBal, ref string errorInfo)
        {

            openingBalExist = true;// don not consider prev day balance as todays opening balance
            MySqlConnection objCon = new MySqlConnection(ConfigValueLoader.ConnectionString);
            ...
            try
            {
                ...
                string query = "SELECT SiNo, OpeningBalance,ClosingBalance, LedgerDate, CreatedBy, CreatedDate, `Column 6`, `Column 7`, `Column 8`, `Column 9`, " +
                    " `Column 10`, `Column 11`, `Column 12` FROM dailyledgerreport where LedgerDate = '" + rptDate + "'";

                MySqlDataAdapter objAdapter = new MySqlDataAdapter(query, objCon);

                objAdapter.Fill(dtledgerRprtDtls);

                if (dtledgerRprtDtls.Rows.Count == 0) // get previous day closing balanace which is equla to todays closing balance
                {

                    query = "SELECT SiNo, OpeningBalance,ClosingBalance, LedgerDate, CreatedBy, CreatedDate, `Column 6`, `Column 7`, `Column 8`, `Column 9`, " +
                       " `Column 10`, `Column 11`, `Column 12` FROM dailyledgerreport where LedgerDate =   subdate('" + rptDate + "', 1)";
                    dtledgerRprtDtls = new DataTable();
                    objAdapter = new MySqlDataAdapter(query, objCon);
                    objAdapter.Fill(dtledgerRprtDtls);

                    if (dtledgerRprtDtls.Rows.Count != 0)
                    {
                        decimal openingBalance = 0;
                        int openingBalanaceSINO = 0;

                        foreach (DataRow ldrRptItem in dtledgerRprtDtls.Rows)
                        {
                            //previous day closing is always todays opening balance
                            openingBalance = Math.Round(Convert.ToDecimal(ldrRptItem["ClosingBalance"]), 2);
                            siNoOfCreatedOpeningBal = Convert.ToInt16(ldrRptItem["SiNo"]);
                            openingBalanaceSINO = 0;
                            //chkOpeningBalanaceExists = true;


                            #region start updating todays ledger balance as yesterdays closing balance
                            LedgerRptDO objLedgerRptDO = new LedgerRptDO();

                            objLedgerRptDO.SiNo = openingBalanaceSINO;
                            objLedgerRptDO.LedgerDate = rptDate;  //dateOpeningBalDate.Text;                         
                            objLedgerRptDO.CreatedBy = "Auto";
                            objLedgerRptDO.CreatedDate = AviMaConstants.CurrentDateTimesStamp;
                            objLedgerRptDO.OpeningBalance = openingBalance;
                            objLedgerRptDO.ClosingBalance = 0;
                            ...
                            #endregion start updating todays ledger balance as yesterdays closing balance


                            #region after auto creating todays opening balance fetch again data for today to get Data table formate
                            // PIP replace this logic to generate data table through code so we can avoid 1 db hit
                                 ...
                                 objAdapter = new MySqlDataAdapter(query, objCon);
                                 objAdapter.Fill(dtledgerRprtDtls);
                            #endregion after auto creating todays opening balance fetch again data for today to get Data table formate


                        }

                        openingBalExist = false; // consider prev day balance as todays opening balance
                    }
                }
            }
            catch (Exception ex)
