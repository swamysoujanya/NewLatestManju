...
{
   public class PrintStkReport
    {
        ReportPrintDO ObjReportPrintDO = null;

        public PrintStkReport(ReportPrintDO _ObjReportPrintDO)
        {
        ...
        private void SetInvoiceData(Graphics g, System.Drawing.Printing.PrintPageEventArgs e)
        {
            ...
            int xDiscount = 425;
            g.DrawString("Supplier", InvoiceFont, BlackBrush, xDiscount + 25, CurrentY - 11);
            ...
            Int64 totalQnty = 0;  // multi page printing
            ...
            for (int i = 0; i < 22; i++)      // multi page printing
            {
                ...
                if (ObjReportPrintDO.RptDetails.Rows.Count > 0)
                {
                    ...
                    g.DrawString(Convert.ToString(item["IBarCode"]), InvoiceFont, BlackBrush, 150, yPosition);
                    g.DrawString(Convert.ToString(item["IName"]), InvoiceFont, BlackBrush, 222, yPosition);
                    g.DrawString(Convert.ToString(item["IQnty"]), InvoiceFont, BlackBrush, 355, yPosition);
                    g.DrawString(Convert.ToString(item["SupplierName"]), InvoiceFont, BlackBrush, 436, yPosition);



                    totalQnty += Convert.ToInt64(item["IQnty"]);
            ...
            grandTotal += totalQnty;  // multi page printing

            //}    

            g.DrawString("Total Qnty", InvoiceFont, BlackBrush, 358, 540);
            g.DrawString(Convert.ToString(totalQnty), InvoiceFont, BlackBrush, 436, 540);  // multi page printing
